...
function buildGeneratorConfigs(componentsCtx, storiesCtx, notesCtx) {
  ...
  return componentKeys.reduce((obj, compKey) => {
    ...
    const noteKey = notesKeys.find(k => k.indexOf(dirName) > -1) || {};
    if(storyKey) {
 ...
 */
function buildStencilStories(name, loader, componentsCtx, storiesCtx, notesCtx) {
	const configs = buildGeneratorConfigs(componentsCtx, storiesCtx, notesCtx);
